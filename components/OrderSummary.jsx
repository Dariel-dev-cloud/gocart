...
const OrderSummary = ({ totalPrice, items = [] }) => {
    ...
    const handleCouponCode = async (event) => {
        ...
        try {
            if (!user) {
                return toast('Por favor inicia sesión para aplicar un cupón');
            }
    ...
    return (
        <div className='w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7'>
            ...
            <div className='flex gap-2 items-center mt-1'>
                ...
                <label htmlFor="STRIPE" className='cursor-pointer'>Pago con Stripe</label>
            </div>
                ...
                <p>Dirección</p>
                {
                    selectedAddress ? (
                        ...
                        <div>
                            ...
                            <button className='flex items-center gap-1 text-slate-600 mt-1' onClick={() => setShowAddressModal(true)} >Agregar Dirección <PlusIcon size={18} /></button>
                        </div>
                    )
                }
            </div>
            ...
            <div className='flex justify-between py-4'>
                ...
                <p className='font-medium text-right'>
                    <Protect plan={'plus'} fallback={`${currency}${coupon ? (totalPrice + 5 - (coupon.discount / 100 *
                        totalPrice)).toFixed(2) : (totalPrice + 5).toLocaleString()}`} >
                        {currency}{coupon ? (totalPrice - (coupon.discount / 100 *
